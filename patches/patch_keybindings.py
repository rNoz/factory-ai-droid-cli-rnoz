...
Rotates three interactive shortcuts without changing binary size:
  ...
  * pull queued messages:  Ctrl-G -> Ctrl-N
...
keeps its guard on Ctrl-N. Machine kebab-case keys (``ctrl-g``) and the
...
TABLE_RE = re.compile(
    ...
    rb".{0,2000}?(?P<editor_key>ctrl-p|ctrl-n)\x00.{0,128}?"
...
DISPATCH_LOCAL_RE = re.compile(
    rb'[A-Za-z_$][A-Za-z0-9_$]*\([^;]{0,100}?"(?P<key>ctrl-[gnp])"\)'
...
def apply_patch_bytes(data: bytes) -> bytes:
    ...
    if editor_key == b"ctrl-n":
        ...
        dispatch_n = _dispatch_matches(data, b"ctrl-n", queue_action)
        if (
            len(dispatch_g) == 1
            ...
            return data
        raise PatchError("keymap is partially patched or has an unknown Ctrl-G/Ctrl-N layout")
    ...
    dispatch_n_any = _dispatch_matches(data, b"ctrl-n")
    if (
        len(dispatch_g) != 1
        or len(dispatch_p) != 1
        ...
        raise PatchError("expected one unique Ctrl-G, Ctrl-P, and Ctrl-N action")
    if dispatch_n_any:
        raise PatchError("upstream already binds Ctrl-N inline; refusing to rotate")
    ...
    new_p = new_p.replace(b'"ctrl-p"', b'"ctrl-n"', 1)
    ...
    # the editor entry itself becomes the queue entry on ctrl-n.
    ...
    patched_keymap = (
        ...
        + ctrl_p_entry.group(0).replace(editor_key, b"ctrl-n").replace(
